...
public class ProductService : GenericService<ProductVm, ProductSaveVm, Product>, IProductService {
  ...
  public ProductService(IProductRepository productRepository, IAccountService userService, IHttpContextAccessor httpContextAccessor, IMapper mapper, IPaymentRepository paymentRepository) : base(productRepository, mapper) {
  ...
  public override async Task Delete(int id) {
    var product = await _productRepository.GetEntity(id);
    if (product.TyAccountId == 3) {
      var payments = await _paymentRepository.GetAll().ContinueWith(t => t.Result.Where(p => p.RProductId == id || p.SProductId == id).ToList());
