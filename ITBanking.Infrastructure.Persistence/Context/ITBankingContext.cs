...
public class ITBankingContext : DbContext
{
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        modelBuilder.Entity<TypeAccount>().ToTable("TypesAccount");
        ...
        modelBuilder.Entity<TypeAccount>().Property(x => x.Id).UseIdentityColumn(1, 1);
        #endregion
        ...
        modelBuilder.Entity<Product>()
          .HasOne(x => x.TypeAccount)
          .WithMany(x => x.Products)
          .HasForeignKey(x => x.TyAccountId).OnDelete(DeleteBehavior.NoAction);

        modelBuilder.Entity<TypeAccount>()
          .HasMany(x => x.Cards)
          .WithOne(x => x.TypeAccount)
          .HasForeignKey(x => x.TyAccountId).OnDelete(DeleteBehavior.Cascade);

        #endregion

        #region Seed
        modelBuilder.Entity<TypeAccount>().HasData(
              new TypeAccount { TypeName = "Cuenta de Ahorro" },
              new TypeAccount { TypeName = "Cuenta de Corriente" },
              new TypeAccount { TypeName = "Cuenta de Cheque" }
            );
        #endregion
    }
}